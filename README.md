...
> Clone it and set it up. Read the README first for instructions, then read the root `index.md` only for the gist, and stop. Wait for questions. When user ask something, navigate using the `index.md` chain: read a subdirectory's `index.md` first, then read only the pages you need.
